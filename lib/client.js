...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LocalModelManager() {
      ...
      function refresh() {
        return call('/status')
          // Deliberately NOT clearing the error here. The 2 s status poll would
          // wipe a failed save/load/restart within one round trip, so the user
          // would see "nothing happened" instead of the reason. The error is
          // cleared where a new operation starts (withBusy) or by the banner's
          // own dismiss button.
      ...
      return h('div', { style: styles.section },
        ...
        editing
          ? h(ModelEditor, {
              // Remount when the edited model changes. Without this React reuses
              // the instance, so its draft state (React.useState(props.initial))
              // keeps the PREVIOUS model's fields while the title shows the new
              // id -- and saving would then write the stale id.
