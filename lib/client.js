  ...
  factory: (require) => {
    ...
    function StatusPanel(props) {
      ...
      return h('div', { style: styles.card },
        ...
        currentModel
          ? h('div', { style: styles.row }, h(Btn, { onClick: function () { props.action('unload'); } }, 'Unload 卸载'),
              h(Btn, { onClick: function () { props.action('restart', currentModel); } }, 'Restart 重启'),
              h(Btn, { onClick: function () { props.action('load', currentModel); } }, 'Load 加载'))
          : null,
        lastError
    ...
    function LocalModelManager() {
      ...
      function action(name, modelId) {
        if (name === 'unload') {
          return withBusy('unload', call('/unload', { method: 'POST', body: {} }).then(function () {
            setNotice('已卸载当前模型，Gateway 仍在运行。');
          }));
        }
        if (name === 'restart') {
